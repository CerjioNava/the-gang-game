...
import type { ConexionCliente, MensajeSaliente } from "../src/servidor/tipos";
...
function montarPartidaEnCurso(
  app: ReturnType<typeof crearAplicacion>,
): JugadorEnPartida[] {
...
describe("aplicación: terminación por desconexión", () => {
  ...
  it("inicia cuenta atrás al desconectar un ladrón en EN_CURSO", () => {
    ...
    expect(vista.terminacionPorDesconexion?.jugadorNombre).toBe(
      desconectado.nombre,
    );
    expect(
      vista.jugadores.find((j) => j.id === desconectado.sessionId)?.conectado,
    ).toBe(false);
  });
  ...
  it("cancela la cuenta atrás si el ladrón reconecta a tiempo", () => {
    ...
    app.manejadores.alDesconectar!(desconectado.conexion);
    expect(
      app.coordinador.obtenerEstado().terminacionPorDesconexion,
    ).not.toBeNull();
